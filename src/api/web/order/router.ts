...
OrderRouter.get('/record', authChecker, async (req: Request, res: Response) => {
  ...
  const maxData = 10;
  const skipData = (currentPage - 1) * maxData;
  let totalPage = 0;
  try {
    const orderDataCountResult = await client.query(
      `SELECT COUNT(CASE WHEN order_status != 1 THEN 1 END) FROM "order"`
    );
    const totalData = Number(orderDataCountResult.rows[0].count);
    totalPage = Math.floor(totalData / maxData) + 1;
    ...
    const orderData = await Promise.all(
      orderResult.rows.map(async (order: IGetOrderRecord) => {
        const detailResult = await client.query(
          `SELECT od.order_id as "orderId", od.detail_id as "detailId", p.name as "productName", p.price as "productPrice", od.count as "productCount", od.created_at as "createdAt"
          FROM order_detail as "od"
          LEFT JOIN product as "p"
    ...
    const data = { orderData, totalPage, totalData };
    return res.status(200).json(data);
  } catch (error) {
