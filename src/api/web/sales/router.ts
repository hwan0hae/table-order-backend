...
SalesRouter.get('/', authChecker, async (req: Request, res: Response) => {
  ...
  try {
    ...
    SELECT to_char(o.modified_at,'DD') AS "day", p.price , od.count 
    FROM order_detail AS "od"
    LEFT JOIN "order" AS "o"
    ON od.order_id = o.order_id
    LEFT JOIN product AS "p"
...
SalesRouter.get('/day', authChecker, async (req: Request, res: Response) => {
  const user = req.currentUser;
  ...
  try {
    const result = await client.query(`
    SELECT p.id AS "productId",p.name AS "productName",  p.price AS "productPrice", SUM(od.count) AS "productCount"
    FROM order_detail AS "od"
    LEFT JOIN "order" AS "o"
    ON od.order_id = o.order_id
    LEFT JOIN product AS "p"
    ON od.product_id = p.id
    WHERE o.order_status=3 AND o.company_id=${user?.company_id} AND o.modified_at BETWEEN '${year}-${month}-${day}' AND '${year}-${month}-${nextDay}'
    GROUP BY p.id
    `);

    return res.status(200).json(result.rows);
  } catch (error: any) {
    console.error('/api/v1/web/sales/day >> ', error);
